            ...
            for (int line = 0; line < value.Count; line++)
            {
                ...
                else
                {
                    ...
                    if (!found)
                    {
                        ...
                        if(ObjectHolder is NodeWallSystem)
                        {
                            Debug.Log("setup was run = " + (ObjectHolder as NodeWallSystem).setupWasRun);

                            parentName = (ObjectHolder as BaseGameObject).transform.name;
                        }
